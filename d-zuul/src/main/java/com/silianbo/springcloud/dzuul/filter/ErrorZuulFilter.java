...
public class ErrorZuulFilter extends ZuulFilter {
    ...
    public String filterType() {
        return ZuulUtil.FilterType.ERROR;
    }
    ...
    public Object run() {
        ...
        if (issuccess) {
            logger.info("{} ErrorZuulFilter request to {}", request.getMethod(), request.getRequestURL().toString());
            logger.info(ctx.getResponseBody());
            ctx.set(ZuulUtil.FilterStatus.ISSUCCESS, true);
        } else {
            ctx.setResponseBody("{\"result\":\"请求Error 过滤器 拦截生效\"}");
